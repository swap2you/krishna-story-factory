...
def repair_story_003_dedup(content: StoryContent) -> StoryContent:
    ...
    # Collapse repeated closing truthfulness paragraphs that restate the same two facts.
    closing_fact = (
        r"Vasudeva brought K[īi]rtim[āa]n[^.?!]*Ka[mṁ]sa[^.?!]*\.[^.?!]*"
        r"Ka[mṁ]sa (?:initially )?returned[^.?!]*\."
    )
    for field_name, blob in (("main", main), ("audio", audio)):
        matches = list(re.finditer(closing_fact, blob, flags=re.I | re.S))
        if len(matches) > 1:
            # Keep the first occurrence; remove later near-duplicates.
            for match in reversed(matches[1:]):
                blob = blob[: match.start()] + blob[match.end() :]
        # Remove adjacent duplicated sentences (same normalized sentence twice).
    ...
    return replace(content, main_story=main, audio_script=audio, story_number=content.story_number or "003")
